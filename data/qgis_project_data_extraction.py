...
def getRasterStatsByLocation(input, zones, min=None, max=None):
    ...
    if (not (min != None and max != None)):
        stats = input.dataProvider().bandStatistics(1, QgsRasterBandStats.All, input.extent(), 0)
    ...
    zones_output = zones.materialize(QgsFeatureRequest().setFilterFids(zones.selectedFeatureIds()))
        ...
        if (isinstance(treeLayer, QgsLayerTreeLayer)):
            ...
            if (layer.name() in mapGroup['vector'].keys()):
                ...
                if ('min' in mapGroup['ramp'][layer.name()] and 'max' in mapGroup['ramp'][layer.name()]):
                    extracted = getRasterStatsByLocation(layer, zones, mapGroup['ramp'][layer.name()]['min'], mapGroup['ramp'][layer.name()]['max'])
                else: 
